...
let handleInvalidAuthHtml: HttpHandler =
    Response.withStatusCode 401 >> Response.ofHtml notAuthorized

let secureHtmlHandler (ok: HttpHandler) : HttpHandler =
    Request.ifAuthenticated ok handleInvalidAuthHtml
...
let fromModel toModel serviceOutput =
    match serviceOutput with
    | Success result -> result |> toModel |> Response.ofJsonOptions jsonOptions
    | Failure e -> errorHandler e
...
module SystemUser =
    let getIntValues (headers: IDictionary<string, string>) name =
        if headers.ContainsKey(name) && headers[name] <> "" then
            try
                headers[name].Split(",") |> List.ofArray |> List.map (fun x -> x |> int)
            with e ->
                List.empty
        else
            List.Empty
...
let getClaims (ctx: HttpContext) =
    Auth.getClaims ctx |> Seq.map (fun c -> (c.Type, c.Value)) |> dict
